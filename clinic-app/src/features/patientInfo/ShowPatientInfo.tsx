...
import { useEditPatientInfo, useShowPatientInfo } from './showPatientInfoAPI';
...
export function ShowPatientInfo() {
    ...
    const editPatientsDetails = useEditPatientInfo(patientId);
    ...
    const handleSaveClick = () => {
        if (editedPatient) {
            ...
            const hasChanges = JSON.stringify(patient) !== JSON.stringify(editedPatient);
    ...
    return (
        (patients as any)?.result?.map((patient: any) => (
            <div>
                <Box justifyContent="center" mt={2}>
                    <div>
                        ...
                        <Box sx={{ width: 1730 }}>
                            <Stack spacing={{ sm: 2, md: 3 }} direction={{ xs: 'column', sm: 'row' }} useFlexGap flexWrap="wrap">
                    ...
                    <Box sx={{ width: 1730, display: 'flex', justifyContent: 'center' }}>
                        <Stack spacing={{ sm: 4, md: 8 }} direction="row">
                            {isEditing ? (
                                <Button variant="contained" disableElevation onClick={handleSaveClick}>
                                    SAVE
                                </Button>
                            ) : (
